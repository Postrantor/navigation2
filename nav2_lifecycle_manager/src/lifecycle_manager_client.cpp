...
/*
  其中 LifecycleManagerClient 类提供了启动、关闭、暂停、恢复和重置生命周期管理器的功能，
  这些功能都是通过调用 callService 函数实现的。
  在构造函数中，会根据节点名称和父节点指针创建服务客户端。每个服务都有一个超时时间，如果在规定时间内没有得到响应，则会返回错误。
*/
...
// 需要弄清楚这里是对 管理器 操作，还是对 被管理节点 的操作
// 启动生命周期管理器
...
/*
  其中包含两个函数，分别为 is_active 和 callService。
 ...
 */
bool LifecycleManagerClient::callService(
    uint8_t command,  //
