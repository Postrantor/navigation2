...
BehaviorServer::BehaviorServer(const rclcpp::NodeOptions &options)
    : LifecycleNode("behavior_server", "", options),
      ...
      default_types_{
          ...
          "nav2_behaviors/Wait"} {
  declare_parameter(
      "local_costmap_topic", rclcpp::ParameterValue(std::string("local_costmap/costmap_raw")));

  declare_parameter(
      "global_costmap_topic", rclcpp::ParameterValue(std::string("global_costmap/costmap_raw")));
...
void BehaviorServer::configureBehaviorPlugins() {
  ...
  for (size_t i = 0; i != behavior_ids_.size(); i++) {
    behaviors_[i]->configure(
        node, behavior_ids_[i], tf_, local_collision_checker_, global_collision_checker_);
  }
}
